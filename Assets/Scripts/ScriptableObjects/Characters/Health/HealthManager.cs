...
namespace ScriptableObjects.Characters.Health
{
    ...
    public class HealthManager : ScriptableObject
    {
        ...
        {
            float multiplier = 1;
            foreach (BuffDebuff buffDebuff in activeBuffDebuffs)
            {
                StatModifier modifier = buffDebuff.StatModifiers.FirstOrDefault(m => m.StatType == statType);
                if (modifier != null) multiplier *= modifier.Multiplier;
            }
        ...
        public void OnTurnStart()
        {
            ...
            foreach (BuffDebuff buffDebuff in activeBuffDebuffs) buffDebuff.Duration--;
            // remove expired
        ...
        public void TakeDamage(HealthManager attacker, int damage, ElementType elementType)
        {
            ...
            if (Random.value < evasionChance && elementType != ElementType.Almighty)
            {
            ...
            if (elementType != ElementType.Almighty)
            {
                ...
                if (!stats.Weaknesses.Contains(elementType)) damage = Mathf.CeilToInt(damage * (1 - CurrentDEF));
                if (isGuarding) damage -= Mathf.RoundToInt(damage * 0.4f);
        ...
        public void ApplyBuffDebuff(BuffDebuff buffDebuff)
        {
            ...
            if (activeBuffDebuffs.Contains(buffDebuff) && !buffDebuff.CanStack) activeBuffDebuffs.Remove(buffDebuff);
